"""Shared Temporal client factory + the start/signal helpers the API uses.
...
async def start_order_workflow(inp: OrderSupervisorInput) -> str:
    """Start the one workflow for this order. Returns its workflow id.
    Raises OrderAlreadySupervised if one already exists."""
    ...
    except WorkflowAlreadyStartedError as exc:  # noqa: TRY003
        raise OrderAlreadySupervised(wf_id) from exc
    return wf_id


def workflow_handle(workflow_id: str) -> WorkflowHandle:
    # get_client() must have been awaited already (it has, if a run exists).
...
async def signal_incoming_event(workflow_id: str, event: dict) -> None:
    ...
    await workflow_handle(workflow_id).signal(
        OrderSupervisorWorkflow.incoming_event, event
    )
...
async def signal_manual_instruction(workflow_id: str, text: str) -> None:
    await get_client()
    await workflow_handle(workflow_id).signal(
        OrderSupervisorWorkflow.manual_instruction, text
    )
...
async def signal_interrupt(workflow_id: str, reason: str) -> None:
    await get_client()
    await workflow_handle(workflow_id).signal(
        OrderSupervisorWorkflow.interrupt, reason
    )
...
async def query_status(workflow_id: str) -> dict | None:
    ...
    try:
        await get_client()
        return await workflow_handle(workflow_id).query(OrderSupervisorWorkflow.status)
    except Exception:  # noqa: BLE001 - best-effort, UI still has the DB row
        return None
