"""asyncpg connection pool, schema bootstrap, and typed query helpers.

Thin data-access layer. The pool is process-global and created once; the
activities and the API routes call the helpers below.
"""
...
# Columns callers may patch on `runs`.
...
async def _init_conn(conn: asyncpg.Connection) -> None:
    """Marshal JSON/JSONB columns to and from python dicts automatically."""
...
async def connect() -> asyncpg.Pool:
    ...
    if _pool is None:
        _pool = await asyncpg.create_pool(
            dsn=settings.database_url, min_size=1, max_size=10, init=_init_conn
        )
...
async def insert_activity(run_id: str, type_: str, payload: dict[str, Any]) -> asyncpg.Record:
    """Append one row to activity_log. Returns id and created_at."""
    async with pool().acquire() as conn:
        return await conn.fetchrow(
            "INSERT INTO activity_log (run_id, type, payload) VALUES ($1, $2, $3) "
...
async def fetch_activities(
    ...
    *,
    ...
    sql = (
        f"SELECT id, run_id, type, payload, created_at FROM activity_log "
...
async def create_run(run_id: str, order_id: str, supervisor_id: str) -> asyncpg.Record:
    """Insert a run row (status 'active', empty memory). The id is supplied by
    the caller so the workflow input can carry it before the workflow starts."""
    async with pool().acquire() as conn:
        return await conn.fetchrow(
            f"INSERT INTO runs (id, order_id, supervisor_id, status, memory_summary) "
            f"VALUES ($1, $2, $3, 'active', '') RETURNING {_RUN_COLS}",
            run_id,
            order_id,
            supervisor_id,
        )
...
async def fetch_run(run_id: str) -> asyncpg.Record | None:
    async with pool().acquire() as conn:
        return await conn.fetchrow(f"SELECT {_RUN_COLS} FROM runs WHERE id = $1", run_id)


async def patch_run(run_id: str, **fields: Any) -> None:
    """UPDATE runs SET <fields> WHERE id = run_id, whitelisted columns only."""
...
async def create_supervisor(
    ...
    async with pool().acquire() as conn:
        return await conn.fetchrow(
            f"INSERT INTO supervisors (name, base_instruction, model_config) "
            f"VALUES ($1, $2, $3) RETURNING {_SUPERVISOR_COLS}",
            name,
            base_instruction,
            model_settings,
        )
...
async def fetch_supervisor(supervisor_id: str) -> asyncpg.Record | None:
    async with pool().acquire() as conn:
        return await conn.fetchrow(
            f"SELECT {_SUPERVISOR_COLS} FROM supervisors WHERE id = $1", supervisor_id
        )
