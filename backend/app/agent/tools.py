...
The 5 required business actions. Each is mocked: "executing" it means writing
one activity_log row (type = agent_action) on the run - no external calls. The
...
_PARAM = {"message": "string - the message / note body"}
...
def tool_specs(allowed: list[str]) -> list[dict[str, Any]]:
    ...
    Unknown names are ignored; empty / missing `allowed` means all 5."""
