...
No LangChain or LlamaIndex. Raw API calls, frozen Pydantic schemas for the
...
| `activity_log` | one append-only log per run; `type` is one of `incoming_event` · `wake_decision` (classifier verdicts *and* agent wake outcomes incl. chosen sleep) · `agent_action` · `manual_instruction` · `final_output`; `payload` JSONB |
...
- `GEMINI_API_KEY` in `backend/.env` for real reasoning. **Leave it blank for
...
copy .env.example .env           # bash/mac: cp .env.example .env  (then set DATABASE_URL and GEMINI_API_KEY)
```

**Fastest, one process** (in-process Temporal, worker, and API, no Temporal CLI needed):

```bash
python -m scripts.dev_stack                       # API on http://localhost:8000  (docs: /docs)
```

**Or the real setup, three terminals** (persistent Temporal plus its web UI on :8233):
...
- **Add instruction**: a run-specific instruction added to a live run's context
- **Lifecycle**: Pause, Resume, Interrupt (an immediate non-terminal wake), and
...
- `paused` is workflow state, not a `runs.status` value (that enum is fixed to
  the four spec values), so it shows on the run-detail page, not the dashboard list.
